...
public abstract class Table<T extends PDPage> {
	 ...
	 */
	@Deprecated
	public Table(float yStart, float yStartNewPage, float pageBottomMargin, float width, float margin, PDDocument document, T currentPage, boolean drawLines, boolean drawContent)
			throws IOException {
	...
	public Table(float yStart, float yStartNewPage, float pageTopMargin, float pageBottomMargin, float width, float margin, PDDocument document, T currentPage, boolean drawLines,
			boolean drawContent, PageProvider<T> pageProvider) throws IOException {
	...
	public Table(float yStartNewPage, float pageTopMargin, float pageBottomMargin, float width, float margin, PDDocument document, boolean drawLines, boolean drawContent,
	...
	public void drawTitle(String title, PDFont[] fonts, int fontSize, float tableWidth, float height, String alignment, float freeSpaceForPageBreak, boolean drawHeaderMargin)
			throws IOException {
		drawTitle(title, fonts, fontSize, tableWidth, height, alignment, freeSpaceForPageBreak, null, drawHeaderMargin);
	}

	public void drawTitle(String title, PDFont[] fonts, int fontSize, float tableWidth, float height, String alignment, float freeSpaceForPageBreak,
		...
		if (title == null) {
			...
			Paragraph paragraph = new Paragraph(title, fonts, fontSize, tableWidth, HorizontalAlignment.get(alignment), wrappingFunction);
			paragraph.setDrawDebug(drawDebug);
			yStart = paragraph.write(articleTitle, margin, yStart);
			if (paragraph.getHeight(fonts) < height) {
				yStart -= (height - paragraph.getHeight(fonts));
			}
	...
	private void drawCellContent(Row<T> row, float rowHeight) throws IOException {
		...
		for (Cell<T> cell : row.getCells()) {
			...
			if (cell instanceof ImageCell) {
				...
				cursorY = yStart - cell.getTopPadding() - (cell.getTopBorder() == null ? 0 : cell.getTopBorder().getWidth());
				...
				if (cell.getFonts() == null && cell.getFonts().length > 0) {
...
//					cursorX += cell.getLeftPadding() + (cell.getLeftBorder() == null ? 0 : cell.getLeftBorder().getWidth())
//							+ FontUtils.getHeight(cell.getFonts(), cell.getFontSize()) + FontUtils.getDescent(cell.getFonts(), cell.getFontSize());
					cursorX += cell.getLeftPadding() + (cell.getLeftBorder() == null ? 0 : cell.getLeftBorder().getWidth())
							+ Paragraph.getAverageFontHeight(cell.getFonts(), cell.getFontSize()) + Paragraph.getAverageDescent(cell.getFonts(), cell.getFontSize());
...
//					cursorY = yStart - cell.getTopPadding() - FontUtils.getHeight(cell.getFont(), cell.getFontSize()) - FontUtils.getDescent(cell.getFont(), cell.getFontSize())
//							- (cell.getTopBorder() == null ? 0 : cell.getTopBorder().getWidth());
					cursorY = yStart - cell.getTopPadding() - Paragraph.getAverageFontHeight(cell.getFonts(), cell.getFontSize())
							- Paragraph.getAverageDescent(cell.getFonts(), cell.getFontSize()) - (cell.getTopBorder() == null ? 0 : cell.getTopBorder().getWidth());
...
//					PDFont currentFont = cell.getParagraph().getDefaultFont(false, false);
					PDFont[] currentFont = cell.getParagraph().getFonts();
					for (Token token : entry.getValue()) {
						...
						case PADDING:
							...
							break;
						case ORDERING:
//							currentFont = new PDFont[] { cell.getParagraph().getDefaultFont(boldCounter > 0, italicCounter > 0) };
							this.tableContentStream.setFonts(cell.getFonts(), cell.getFontSize());
							if (cell.isTextRotated()) {
							...
							break;
						case BULLET:
							float widthOfSpace = currentFont[0].getSpaceWidth();
//							float halfHeight = FontUtils.getHeight(currentFont, cell.getFontSize()) / 2;
							float halfHeight = Paragraph.getAverageFontHeight(currentFont, cell.getFontSize()) / 2;
							if (cell.isTextRotated()) {
								PDStreamUtils.rect(tableContentStream, cursorX + halfHeight, cursorY, token.getWidth(currentFont) / 1000 * cell.getFontSize(),
										widthOfSpace / 1000 * cell.getFontSize(), cell.getTextColor());
								// move cursorY for two characters (one for
								// bullet, one for space after bullet)
								cursorY += 2 * widthOfSpace / 1000 * cell.getFontSize();
							} else {
								PDStreamUtils.rect(tableContentStream, cursorX, cursorY + halfHeight, token.getWidth(currentFont) / 1000 * cell.getFontSize(),
										widthOfSpace / 1000 * cell.getFontSize(), cell.getTextColor());
...
//							currentFont = cell.getParagraph().getFont(boldCounter > 0, italicCounter > 0);
					...
					if (cell.isTextRotated()) {
						cursorX = cursorX + cell.getParagraph().getAverageFontHeight() * cell.getLineSpacing();
					} else {
						cursorY = cursorY - cell.getParagraph().getAverageFontHeight() * cell.getLineSpacing();
					}
				}
			}
	...
	private void drawCellBorders(float rowHeight, Cell<T> cell, float xStart) throws IOException {
		...
		if (bottomBorder != null) {
			...
			drawLine(xStart, y, xStart + cell.getWidth() - (rightBorder == null ? 0 : rightBorder.getWidth()), y, bottomBorder);
		}
	 ...
	 * /**
	 *
	 * @deprecated Use {@link #addHeaderRow(Row)} instead, as it supports multiple
	 *             header rows
